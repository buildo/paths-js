(function() {
  ...
  describe('bar chart', function() {
    ...
    return describe('stacked', function() {
      return it('should give access to the original items', function() {
        var bar1;
        bar1 = Bar({
          stacked: true,
          data: data,
        ...
        return expect(bar1.curves[22].item).to.be(data[2][4]);
      });
    });
  });
  ...
  describe('bar chart axes', function() {
    return it('should compute y axis values with requested step', function() {
      var bar1;
      bar1 = Bar({
        data: data,
        width: 300,
      ...
      expect(bar1.y).not.to.be(void 0);
      return expect(bar1.y).to.eql([0, 2, 4, 6, 8]);
    });
  });
